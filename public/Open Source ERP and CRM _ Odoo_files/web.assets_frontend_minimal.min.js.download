...
odoo.define('@website/js/content/redirect',['@web/session'],async function(require){'use strict';let __exports={};const{session}=require('@web/session');document.addEventListener('DOMContentLoaded',()=>{if(session.is_website_user){return;}
...
/* /openerp_crm/static/src/js/content/inject_dom.js */
odoo.define('@openerp_crm/js/content/inject_dom',['web.utils.cookies'],async function(require){'use strict';let __exports={};const{getCookie}=require('web.utils.cookies');document.addEventListener('DOMContentLoaded',()=>{const htmlEl=document.documentElement;const cookieNamesToDataNames={'utm_source':'utmSource','utm_medium':'utmMedium','utm_campaign':'utmCampaign',};for(const[name,dsName]of Object.entries(cookieNamesToDataNames)){const cookie=getCookie(name);if(cookie){htmlEl.dataset[dsName]=cookie.replace(/(^["']|["']$)/g,'');}}});return __exports;});